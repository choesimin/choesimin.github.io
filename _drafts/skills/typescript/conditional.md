...
- 조건부 타입을 제네릭 타입과 함께 사용할 때는 `T extends U ? X : Y` 형태의 구문을 사용하여, **타입 `T`가 `U`를 확장(상속)한다면 `X` 타입을, 그렇지 않다면 `Y` 타입을 결과로 반환**합니다.
...
- `createLabel` 함수는 입력 값의 타입에 따라 반환 값의 타입이 달라지기 때문에, 여러 개의 함수를 만들어 overloading합니다.
...
- 함수를 직접 선언하여 overloading하는 방식을 사용하면 program 전체에서 매번 비슷한 종류의 함수를 만들어야 합니다.
    - `createLabel` 함수에서 새로운 타입을 다루려면, overloading 함수의 수는 기하급수적으로 증가하게 됩니다.
    - 이는 번거로운 작업 중 하나입니다.
...
- 조건부 타입을 사용하면 단일 함수까지 overloading 없이 단순화시킬 수 있습니다.


### 재귀 참조 불가

- union type, intersectino type과 유사하게, **조건부 타입은 재귀적으로 자기 자신을 참조할 수 없습니다.**
...
- 조건부 타입이 제네릭 타입과 함께 사용될 때, **type 인수(argument)로 유니온 타입을 받으면 분산적으로 동작**하게 됩니다.
    - 각 요소에 대해 개별적으로 조건을 평가하여 결과 타입을 결정합니다.

- 분산 조건부 타입은 인스턴스를 생성하는 과정에서 자동으로 유니언 타입으로 분산됩니다.
    - `T`에 대한 타입 인수로 `A | B | C`를 사용하여 `T extends U ? X : Y` type을 인스턴스로 만들면, `(A extends U ? X : Y) | (B extends U ? X : Y) | (C extends U ? X : Y)` 변환되어 type이 결정됩니다.
...
1. `ToArray` 조건부 type을 적용시기키 위해, type 인수로 받은 union type(`string | number`)을 가져옵니다.
...
3. 분산성이 적용된 결과인 **`string[] | number[]`를 최종적으로 반환**합니다.
...
### 분산 조건부 Type 활용 예제 : 유니언 타입 Filtering
...
## 조건부 Type의 Type 추론 : `infer` 키워드

- `infer` 키워드는 TypeScript의 고급 타입 기능 중 하나로, **조건부 타입(`T extends U ? X : Y`) 내에서 사용되어 타입을 동적으로 추론**하는 데 사용됩니다.
    - `infer` 키워드를 통해 조건부 type의 타입 추론(type inference) 기능을 사용할 수 있으며, **'참' 값 분기에서 비교하는 타입을 추론**합니다.

- `infer` 키워드는 주로 제네릭 타입과 함께 사용되며, 특정 타입의 구조에서 일부를 동적으로 추론할 때 유용합니다.
    - e.g., 함수 타입의 매개 변수나 반환 타입을 추론하거나, 제네릭 타입에서 특정 타입을 추출하는 데 사용됩니다.
...
- 조건부 타입을 사용할 때, `T extends U ? X : Y` 형태의 조건문에서 `U`의 위치에 `infer` 키워드를 사용하여 타입을 추론할 수 있습니다.
    - `infer` 키워드 다음에 나오는 변수(e.g., `infer R`)는 조건부 타입이 참인 경우에만 타입을 추론하며, 이 변수는 추론된 타입을 나타내는 데 사용됩니다.

- `infer` 키워드의 사용은 복잡한 타입 조작을 단순화하고, 코드의 가독성을 높이며, 재사용 가능한 타입을 생성하는 데 큰 도움을 줍니다.
    1. **복잡한 타입 추론 가능** : `infer`를 사용하면, 복잡한 구조에서 특정 타입을 추출하거나, 함수의 반환 타입, 튜플의 요소 타입 등을 쉽게 추론할 수 있습니다.
    2. **가독성 향상** : 복잡한 타입 연산을 명확하고 간결한 방식으로 표현할 수 있어, 타입 정의를 이해하기 쉬워집니다.
    3. **재사용성 증가** : 추론된 타입을 다양한 곳에서 재사용할 수 있어, 타입 코드의 중복을 줄이고 유지 보수성을 향상시킵니다.

- `infer` 키워드는 조건부 타입의 `true` 분기에서만 사용할 수 있습니다.
- 추론할 타입이 명확하지 않거나 조건부 타입이 항상 참이 아닌 경우, 보통 `never` 타입을 반환합니다.
...
- `ElementType<T>` 타입은 배열 `T`의 요소 타입을 추론합니다.
    - `T`가 배열이라면, `infer U`를 사용하여 배열의 요소 타입을 `U`로 추론하고, 그 타입을 반환합니다.
    - 배열이 아닌 경우에는 `never` 타입을 반환합니다.
...
- `ReturnType<T>` 조건부 타입을 사용하여, 제네릭 타입 `T`가 함수 타입일 경우 그 함수의 반환 타입을 추론합니다.
    - `T`가 함수 타입이라면, 해당 함수의 반환 타입을 `infer R`를 사용하여 `R`로 추론하고, 그 타입을 반환합니다.
    - 함수 타입이 아닌 경우에는 `never`를 반환합니다.

- `T extends (...args: any[]) => infer R ? R : never` 구문에서 `infer R` 부분은 `T` 타입의 함수가 반환하는 타입을 `R`로 추론하라는 의미입니다.
...
- `UnwrapPromise<T>` 타입은 `T`가 `Promise`의 인스턴스일 경우, 프로미스가 해결(`resolve`)될 때의 타입을 추론합니다.
    - 만약 `T`가 `Promise` 타입이라면, 그 안에 감싸진 타입을 `infer U`를 사용하여 추론하고 반환합니다.
    - `T`가 프로미스가 아니라면, `T` 자체를 반환합니다.
...
- `PropertyType<T, K>` 타입은 객체 타입 `T`에서 키 `K`에 해당하는 프로퍼티의 타입을 추론합니다.
    - `PropertyType<T, K>` 타입은 객체 `T`가 키 `K`를 가지고 있고, 해당 키의 타입을 `infer U`를 통해 `U`로 추론할 수 있다면, 그 타입을 반환합니다.
    - 만약 `T`에 `K` 키가 없다면, `never` 타입을 반환합니다.
