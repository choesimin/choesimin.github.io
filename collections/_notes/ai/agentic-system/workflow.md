...
- workflow는 task가 사전에 분해 가능할 때 효과적입니다.
    - task의 단계와 분기 구조가 안정적일수록 효율이 높아지며, 자율성을 LLM에 넘기지 않으므로 오류 누적 위험도 낮습니다.
...
- marketing copy 생성 후 다국어로 번역하는 흐름이 대표적입니다.
    - 첫 호출은 영문 copy를 생성하고, 두 번째 호출은 그 copy를 한국어로 번역하며, 각 단계는 명확히 분리됩니다.
...
- 고객 문의 system이 대표적인 routing 사례입니다.
...
- input이 명확히 구분되는 category로 나뉘고, 각 category가 다른 처리 logic을 요구할 때 적합합니다.
    - 한 prompt로 모든 case를 다루려 하면 prompt가 비대해지고 정확도가 떨어지므로, 분리해서 각자 단순한 prompt로 처리하는 게 효과적입니다.
...
- 문서 분석에서 주제 추출, 감성 분석, 사실 검증을 동시에 실행하는 경우가 sectioning의 전형입니다.
    - 세 작업은 서로 의존하지 않으므로 병렬 실행이 자연스럽고, latency가 단일 호출 수준으로 떨어집니다.
...
- 평가 기준이 다른 reviewer를 각각 두고 합의를 거치는 방식도 voting의 일종입니다.


### 적합한 상황

- sub-task가 서로 독립적이고 latency 단축이 중요한 경우 sectioning이 적합합니다.
    - 순차로 처리해도 결과는 같지만, 시간이 길어지는 task에서 효과가 큽니다.
...
- parallelization은 **sub-task가 설계 시 고정**되고, orchestrator-workers는 **sub-task가 실행 시 LLM에 의해 결정**됩니다.
...
- 동적 분해 능력이 orchestrator-workers를 agent에 가깝게 만들지만, **종료 조건이 LLM의 종료 신호가 아니라 "분해된 모든 sub-task의 완료"**라는 점에서 여전히 workflow입니다.
    - 반복 횟수와 경로는 LLM이 정하지만, 흐름 자체는 "분해 -> 실행 -> 종합" 단계로 code에 사전 정의되어 있습니다.


### 적용 예시

- multi-file code 수정이 orchestrator-workers의 대표 사례입니다.
    - orchestrator가 "이 변경은 file A, B, C를 수정해야 한다"고 결정한 뒤, 각 file 수정을 worker에 위임하고 결과를 합칩니다.
    - 수정할 file 수가 입력마다 다르므로 사전 정의가 불가능하며, 동적 분해가 필요합니다.
...
- 긴 문서를 여러 section으로 나눠 각각 요약하고 합치는 경우도, section 경계가 가변적이면 orchestrator-workers입니다.


### 적합한 상황

- sub-task의 수와 내용을 사전에 예측할 수 없으나, 전체 흐름(분해 -> 위임 -> 종합)은 고정인 경우 적합합니다.
    - sub-task 자체는 동적이지만, 흐름이 사전 정의되어 있으므로 agent보다 비용과 결과 예측성이 좋습니다.
...
- 문학 번역이 evaluator-optimizer의 전형입니다.
...
- pattern은 sub-task가 언제 결정되는지, 반복이 필요한지, 결과를 어떻게 합치는지로 구분됩니다.

| Pattern | sub-task 결정 시점 | 반복 여부 | 결과 합치는 방식 |
| --- | --- | --- | --- |
| **Prompt Chaining** | 사전 정의 | 없음 | 마지막 단계의 출력 |
| **Routing** | 사전 정의 | 없음 | 선택된 path의 출력 |
| **Parallelization** | 사전 정의 | 없음 | fan-in으로 통합 |
| **Orchestrator-Workers** | 실행 시 동적 결정 | 없음 | orchestrator가 종합 |
| **Evaluator-Optimizer** | 사전 정의 | 있음 | 통과 시점의 generator 출력 |
...
- pattern은 서로 조합할 수 있습니다.
    - routing으로 분기된 한 path 안에서 prompt chaining을 돌리거나, prompt chaining의 한 단계가 parallelization으로 구현되는 경우가 흔합니다.
...
- workflow의 한계에 도달했음을 알려주는 신호가 있습니다.
