...
- 영어 단어의 대소문자는 **위치(제목, table header, 본문)**와 **명사 종류(고유 명사, 일반 명사)**에 따라 다르게 적용합니다.
...
- 모든 제목은 반드시 하위 내용을 포함해야 합니다.
    ...
    - 제목 바로 아래의 개괄 설명은 추상적이거나 형식적인 문장이 아니라, 해당 section의 핵심 내용을 구체적으로 요약해야 합니다.
        ...
        - 추가 예시 : 하위 section에서 다루는 내용을 구체적으로 명시합니다.
            ...
            - "LSM Tree와 B-Tree 기반 storage engine의 특성을 비교합니다." → "LSM Tree는 **write 성능과 space efficiency**에서 우수하고, B-Tree는 **read 성능**에서 우수합니다.".
            - "GTID 환경을 운영할 때 주의할 점입니다." → "GTID 환경에서는 **binary log 보관 기간, backup 복원 시 gtid_purged 설정, errant transaction 방지**를 관리해야 합니다.".
...
- table에서 row header 역할을 하거나 핵심 정보를 담은 column은 bold 처리합니다.
    - 대부분 첫 번째 column이지만, 핵심 column이 다른 위치에 있을 수도 있습니다.
    - 예 : 항목명, 이름, 분류 등.
