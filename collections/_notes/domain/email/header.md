...
- **To field**는 주요 수신자의 email 주소를 지정합니다.
    ...
    - 수신자가 여러 명일 경우 대량 발송 spam일 가능성을 의심해볼 필요가 있습니다.
...
- **Sender field**는 실제로 email을 발송한 주체를 명시합니다.
    ...
    - email 인증과 spam 방지를 위한 추가적인 검증 요소로 작용합니다.
...
- **Subject field**는 email의 주제와 내용을 요약합니다.
    ...
    - "긴급", "당첨", "무료" 등 click을 유도하는 의심스러운 문구는 spam일 가능성이 높습니다.
...
- **Date field**는 email이 작성되고 발송된 정확한 시간을 기록합니다.
    ...
    - 긴급성 판단과 적절한 응답 시기 결정에 활용됩니다.
    - 발신자와 수신자의 time zone이 불일치하거나 시간이 부정확할 경우 의심스러운 email일 수 있습니다.
...
- **Message-ID field**는 각 email의 고유한 식별자를 제공합니다.
    ...
    - `1234567890@example.com` 형식으로 표시되며, 중복되거나 이상한 형식일 경우 suspicious한 email일 수 있습니다.


### Thread 관리 : References

- **References field**는 email thread의 연결 정보를 관리합니다.
    - 답장이나 전달 시 이전 message들의 Message-ID를 포함합니다.
...
- **User-Agent field**는 email을 작성한 client program의 정보를 표시합니다.
    ...
    - 발신자가 사용한 client를 확인하여 이상한 client가 사용된 경우 suspicious한 email일 수 있습니다.
...
- **MIME-Version field**는 사용된 MIME 표준의 version을 명시합니다.
    ...
    - email에 첨부 파일이나 multimedia content가 포함될 때 필수적입니다.
    - email client가 message를 올바르게 해석하기 위한 기준 정보를 제공합니다.
    - text만 포함된 간단한 email에서도 호환성을 위해 포함되는 경우가 많습니다.
    - 일반적으로 "1.0" 이외의 다른 version이 사용될 경우 의심스러운 email일 수 있습니다.
...
- **Content-Type field**는 email body의 형식과 encoding 정보를 지정합니다.
    ...
    - spam email은 종종 HTML을 사용하여 숨겨진 link를 포함시키므로 HTML 형식일 때 주의가 필요합니다.
