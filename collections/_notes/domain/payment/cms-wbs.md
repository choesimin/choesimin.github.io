...
### `cms` Table : CMS 결제를 위한 고객의 계좌 정보

- `cms` table에서 고객의 계좌 정보를 저장하고 있음

- `cms_trace_key` column는 사용자를 특정할 수 있는 KSNET의 식별 값
    - KSNET에 계좌가 성공적으로 등록되면 이 값을 응답으로 받음


### `pay_type_mapping` Table의 `ksnet_cms_wbs_` Prefix를 가진 Column들 : 모계좌 정보

```sql
select
    ksnet_cms_wbs_ekey,
    ksnet_cms_wbs_msalt,
    ksnet_cms_wbs_kscode,
    ksnet_cms_wbs_account_num,
    ksnet_cms_wbs_bank_cd,
    ksnet_cms_comp_code
from pay_type_mapping
where pay_type_idx = 'CMS';
```

- `pay_type_mapping` table의 `ksnet_cms_wbs_` prefix를 가진 column은 CMS 거래에서 고객이 결제한 금액이 도착하는 **기업의 모계좌 정보를 저장**하고 있음
