...
- 네트워크 계층은 여러 개의 노드를 거칠 때마다 경로를 찾아주는 역할을 하는 계층입니다.
    - 다양한 길이의 데이터를 네트워크들을 통해 전달하고, 그 과정에서 전송 계층이 요구하는 서비스 품질(QoS)을 제공하기 위한 기능적, 절차적 수단을 제공합니다.
...
- 네트워크 계층은 인터넷 프로토콜 스위트(TCP/IP)의 핵심 계층 중 하나로, **서로 다른 네트워크에 있는 장치들 간의 데이터 전송을 가능하게 하는 역할**을 합니다.
    - 네트워크 계층은 IP 주소를 이용한 논리적인 주소 지정, 라우팅을 통한 경로 설정, 패킷 분할 및 재조립, 오류 감지 및 처리 등의 기능을 통해 **서로 다른 네트워크에 있는 장치들 간의 데이터 전송을 가능하게 하고, 인터넷이라는 거대한 네트워크를 구축하고 운영하는 데 핵심적인 역할**을 합니다. 

- 네트워크 계층의 핵심 프로토콜은 **IP (Internet Protocol)**입니다. IP는 데이터를 패킷 형태로 캡슐화하고, 각 패킷에 출발지 및 목적지 IP 주소를 포함시켜 라우터들이 패킷을 목적지까지 전달할 수 있도록 합니다.
- 네트워크 계층은 OSI 7계층 모델의 3계층에 해당하며, 데이터 링크 계층과 전송 계층 사이에 위치합니다. 데이터 링크 계층은 물리적인 네트워크 연결을 담당하고, 전송 계층은 데이터의 안정적인 전송을 담당합니다.

- 네트워크 계층은 라우팅, 패킷 포워딩, Segmentation/Desegmentation, Internetworking 등을 수행한다.
    - 네트워크 계층의 대표적인 장비는 라우터이며, 최근에는 2계층 장비인 스위치에 라우팅 기능을 장착한 Layer 3 스위치도 있다.
        <!-- - 라우터에서 IP 주소를 활용하며, 특정 지점까지의 경로를 찾은 것을
        - 참고로 네이버나 구글 같은 도메인 주소를 55.10.54.75 같은 IP 주소로 변환하는 행위를 DNS라고 한다.
        - B라는 지점에 IP 주소인 55.10.54.75를 부여하고, A에서 B까지 경로를 찾기 위한 라우팅 기술을 사용하는 것이다. -->

3/4계층은 TCP/IP 프로토콜 스택은 컴퓨터 커널 내부에 구현되며, 하위 2계층과 디바이스 드라이버(네트워크 드라이버)를 통해 컴퓨터 내부에서 데이터를 통신한다.
선결조건으로 운영체제에서 네트워크 장치(모뎀, 랜카드)가 사전에 인식되어 있어야 한다.
TCP/IP 프로토콜 스택은 2계층에서 받은 데이터(프레임)을 소켓 인터페이스를 통해 7계층(응용계층, TCP/IP 4계층)으로 올려보낸다.

- 서브네트의 최상위 계층으로 경로를 설정하고, 청구 정보를 관리한다.
    - 주소 부여(IP), 경로 설정(Route)
    - 개방형 시스템들의 사이에서 네트워크 연결을 설정, 유지, 해제하는 기능을 부여하고, 전송 계층(트랜스포트 계층) 사이에 네트워크 서비스 데이터 유닛(NSDU: Network Service Data Unit)을 교환하는 기능을 제공한다.

- 현재 사용하는 인터넷은 기본적으로는 네트워크 계층에 속하는 망이다.
    - L3, L4, L5는 보통 국제 인터넷 표준화 기구(IETF)에서 규격을 관리한다.
...
1. **논리적인 주소 지정 (IP 주소)** : 네트워크 계층은 각 장치에 고유한 IP 주소를 할당하여 인터넷 상에서 식별 가능하도록 합니다.
    - 이 IP 주소는 데이터를 목적지까지 정확하게 전달하는 데 사용됩니다.

2. **경로 설정 (라우팅)** : 네트워크 계층은 라우팅 프로토콜을 사용하여 최적의 경로를 찾아 데이터를 목적지까지 전달합니다.
    - 인터넷은 수많은 네트워크들이 복잡하게 연결되어 있는데, 라우팅은 이러한 복잡한 네트워크 환경에서 데이터가 올바른 경로를 통해 효율적으로 전달되도록 보장합니다.

3. **패킷 분할 및 재조립** : 네트워크 계층은 데이터를 작은 단위인 패킷으로 분할하여 전송하고, 목적지에 도착하면 다시 원래의 데이터로 재조립합니다.
    - 이를 통해 다양한 크기의 데이터를 효율적으로 전송하고, 네트워크 혼잡을 줄일 수 있습니다.

4. **오류 감지 및 처리** : 네트워크 계층은 데이터 전송 중 발생할 수 있는 오류를 감지하고, 필요한 경우 재전송을 요청하여 데이터의 신뢰성을 보장합니다.
...
- 네트워크 관리자가 직접 주소를 할당하는 **논리적인 주소 구조(IP)**를 가지며, IP의 구조는 계층적(hierarchical)입니다.
