...
- 데이터 링크 계층은 물리 계층 위에서 동작하며, **같은 네트워크 내 장치들 간의 데이터 전송을 담당**합니다.

<!-- - 데이터 링크 계층은 같은 네트워크 내 장치들 간 데이터 전송을 담당하는 계층입니다. -->


- 데이터 링크 계층은 물리적인 신호 전달을 넘어, **안정적이고 효율적인 데이터 전송을 가능하게 하는 핵심적인 역할**을 수행합니다.
    - 네트워크 통신의 기본 토대를 제공하며, 상위 계층들이 원활하게 동작할 수 있도록 지원합니다. 
...
- 데이터 링크 계층은 **네트워크 연결의 핵심**이며, **상위 계층들이 효과적으로 기능할 수 있는 기반**을 제공합니다. 
    ...
    - **네트워크 연결의 기본** : 물리 계층과 네트워크 계층 사이의 핵심 연결 고리 역할을 합니다.
    - **데이터 전송의 신뢰성** : 오류 검출 및 수정, 흐름 제어 등을 통해 데이터 손실을 최소화하고 안정적인 통신을 가능하게 합니다.




**1. 물리 계층과의 차이: 신호 vs 데이터**
...
**2. 네트워크 계층과의 차이: 로컬 vs 글로벌**

* **네트워크 계층**: 다른 네트워크로 데이터를 전달하는 라우팅 기능을 담당하며, 논리적인 주소(IP 주소)를 사용합니다. 
* **데이터 링크 계층**: 같은 네트워크 내 장치들 간의 통신을 담당하며, 물리적인 주소(MAC 주소)를 사용합니다. 즉, 네트워크 계층이 '큰 그림'을 그린다면, 데이터 링크 계층은 그 그림을 실제로 구현하는 '붓'과 같습니다.

**3. 전송 계층과의 차이: 연결 vs 신뢰성**
...
## 데이터 링크 계층: 택배 상자 포장 및 주소 라벨 붙이기 담당! 

**어려운 설명:**

> 일반적으로 물리 계층에서 구현된 한 단위의 데이터인 프레임을 받아 통신 링크를 따라 전송 가능한 형태로 변환하는 역할로 비트의 흐름을 전자기 또는 광 신호로 변환 후 매체를 통해 신호를 전송하는 역할을 한다.

**쉬운 설명:**

인터넷에서 데이터를 주고받을 때, 데이터는 컴퓨터가 이해할 수 있는 0과 1의 형태로 되어 있습니다. 이 데이터들을 **택배 상자**라고 생각해 봅시다. 데이터 링크 계층은 이 택배 상자를 **포장하고**, **받는 사람 주소(MAC 주소)**를 써서 **배달 가능한 형태**로 만드는 역할을 합니다. 

**어려운 설명:**

> 주로 사용되는 장비에는 데이터 단말 장비(Data Terminal Equipment, DTE)와 데이터 통신 장비(Data Communication Equipment, DCE)가 있다.

**쉬운 설명:**

* **데이터 단말 장비 (DTE):** 택배를 보내거나 받는 **컴퓨터**라고 생각하면 됩니다.
* **데이터 통신 장비 (DCE):** 택배를 **중간에서 전달**해주는 **우체국** 또는 **택배 기사**라고 생각하면 됩니다.

**결론:**

데이터 링크 계층은 인터넷 통신에서 데이터를 안전하게 전달하기 위한 중요한 역할을 합니다. 마치 택배를 보낼 때 꼼꼼하게 포장하고 주소를 정확히 쓰는 것처럼, 데이터 링크 계층은 데이터를 올바른 형태로 만들어 목적지까지 안전하게 도착하도록 돕습니다. 
