...
이 동영상은 네트워크 참조 모델, OSI 7 계층 모델 및 TCP/IP 4 계층 모델에 관한 것입니다. 강사는 이러한 모델이 실제 네트워크 개발에는 실용적이지 않다고 주장합니다.

강사는 사람들이 네트워크 참조 모델을 네트워크의 핵심 지식으로 오해하고 각 계층의 기능을 암기하려고 한다고 주장합니다. 그러나 이러한 모델은 네트워크를 설계하고 구성하기 위한 지침이나 참조에 더 가깝고, 네트워크 장치가 엄격하게 따르는 실제 규칙이 아닙니다.

강사는 자신의 주장을 뒷받침하기 위해 몇 가지 주장을 제시합니다. 첫째, 새로운 네트워크 프로토콜과 장치가 끊임없이 개발되고 있으며, 이러한 프로토콜과 장치는 OSI 또는 TCP/IP 모델의 특정 계층에 완벽하게 들어맞지 않는 경우가 많습니다. 둘째, 네트워크 장치는 종종 참조 모델의 여러 계층에 걸친 기능을 수행합니다. 예를 들어, 3계층 장치인 라우터는 2계층 작업도 수행할 수 있습니다.

강사는 네트워크 참조 모델이 유용한 개념적 도구이지만 네트워크 개발 작업에서는 실제 프로토콜과 네트워크 장치를 이해하는 것이 더 중요하다고 결론짓습니다. 예를 들어, 웹 개발자는 HTTP 프로토콜을 자세히 이해해야 하지만 OSI 애플리케이션 계층 프로토콜의 이름을 암기하는 것은 별로 도움이 되지 않습니다.

요약하자면, 이 동영상은 네트워크 참조 모델이 네트워크 개념을 이해하는 데는 유용하지만 실제 네트워크 개발 작업에는 유용하지 않다고 주장합니다.
