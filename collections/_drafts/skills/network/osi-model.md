...
- data가 계층을 지날 때마다 각 계층의 전송 측에서 header가 붙게 되고, 수신 측은 역순으로 header를 분석합니다.
    - OSI 모델의 각 계층은 데이터 전송을 위해 각기 다른 기능을 수행하며, 각 계층을 지날 때마다 추가 정보(header)가 붙습니다.

| 계층 | Data 단위 | Data 구성 |
| --- | --- | --- |
| **L7. 응용 계층** | Message or Data | 실제 사용자 데이터 |
| **L6. 표현 계층** | Message or Data | 인코딩, 압축, 암호화 등이 적용된 형태의 Data |
| **L5. 세션 계층** | Message or Data | 세션 정보가 추가된 Data |
| **L4. 전송 계층** | Segment | Data / **TCP Header** (또는 **UDP Header**) |
| **L3. 네트워크 계층** | Packet | Data / TCP Header / **IP Header** |
| **L2. 데이터 링크 계층** | Frame | Data / TCP Header / IP Header / **MAC 주소** |
| **L1. 물리 계층** | Bit | 101010101010... (전기 신호, 광신호 등) |

- Header는 네트워크 통신에서 데이터가 올바르게 전송되고 수신될 수 있도록, 각 계층에서 데이터를 처리하는 데 필요한 **제어 정보**를 포함하는 부분입니다.
    - **헤더**는 데이터에 추가되는 **메타데이터**로, 데이터를 어떻게 처리하고 전송해야 하는지에 대한 정보를 담고 있습니다.
        - 헤더는 데이터를 전송할 때 **발신자**와 **수신자** 간에 원활하게 소통할 수 있도록 정보를 제공합니다.
    - 데이터를 **어디로 보낼지**, **어떤 방법으로 전송할지**, **오류가 발생하면 어떻게 처리할지** 등의 다양한 제어 정보를 포함합니다.
    - 각 계층에서 헤더는 특정한 역할을 수행하며, 이러한 정보가 없다면 데이터는 목적지에 제대로 도착하지 않거나, 도착하더라도 올바르게 해석될 수 없습니다. 

- 각 계층에서 헤더는 다른 역할을 하지만, 공통적으로 다음과 같은 **제어 정보**를 포함합니다.
    - **주소 정보** : 데이터가 어디로 보내져야 하는지, 그리고 어디에서 왔는지를 알려주는 정보입니다.
        - 예를 들어, 네트워크 계층의 헤더에는 IP 주소가 포함되어 있어, 데이터가 정확한 목적지로 전달되도록 도와줍니다.
    - **에러 처리 정보** : 데이터 전송 중에 발생할 수 있는 오류를 탐지하고 처리할 수 있는 정보입니다.
        - 예를 들어, 전송 계층의 TCP 헤더는 **체크섬**과 같은 오류 감지 코드를 포함해 데이터 손상이 있는지 확인합니다.
    - **전송 및 흐름 제어 정보** : 데이터가 수신자에게 어떻게 도달해야 하는지, 그리고 얼마나 빠르게 전송되어야 하는지를 제어하는 정보입니다.
        - 예를 들어, TCP 헤더에는 **시퀀스 번호**가 포함되어 있어 데이터가 순서대로 전송되도록 보장합니다.
    - **프로토콜 정보** : 상위 계층에서 사용하는 프로토콜이 무엇인지 알려주는 정보입니다.
        - 예를 들어, IP 헤더에는 **상위 프로토콜 번호**가 있어, 해당 패킷이 TCP, UDP 등의 어떤 프로토콜을 사용하는지 알려줍니다.

- OSI 7계층에서 각 계층마다 헤더는 각기 다른 역할을 수행하며, 이를 통해 전체 데이터 전송 과정이 원활하게 이루어집니다.
    - **응용 계층(L7)** : 사용자가 사용하는 응용 프로그램에 관련된 정보(HTTP 요청, 이메일 프로토콜 등)를 포함할 수 있습니다.
    - **전송 계층(L4)** : TCP나 UDP 헤더는 데이터가 올바른 순서로 전송되고, 손실 없이 도착하도록 제어합니다.
        - 여기에는 포트 번호, 시퀀스 번호, 오류 검출 정보 등이 포함됩니다.
    - **네트워크 계층(L3)** : IP 헤더는 데이터가 올바른 네트워크 주소로 전달되도록 하는데, 발신지와 목적지의 IP 주소를 포함합니다.
    - **데이터 링크 계층(L2)** : 이더넷 헤더는 네트워크 내에서 물리적인 장치 간 데이터를 전달하는 역할을 하며, MAC 주소와 같은 정보가 들어 있습니다.

- 수신 측에서는 데이터를 받으면 각 계층에서 헤더를 분석하여 데이터를 해석하고 처리합니다.
    - 수신 측에서는 물리 계층(L1)부터 데이터를 받아오며, 각 계층에서 이전 계층의 Header를 하나씩 제거하면서 데이터를 처리합니다.
        - 마지막으로 응용 계층(L7)에서 최종적으로 사용자에게 데이터를 보여줍니다.
    - 수신자는 헤더에 포함된 제어 정보를 바탕으로 **데이터가 손상되었는지 확인**하고, **어느 응용 프로그램으로 데이터를 전달해야 하는지 결정**하며, **데이터의 순서를 맞추는** 등 올바른 처리를 할 수 있게 됩니다.
...
- 응용 계층은 **응용 프로세스와 직접 관계하여 일반적인 응용 서비스를 수행**하며, 사용자 또는 애플리케이션이 네트워크에 접근할 수 있도록 합니다.
    - 응용 서비스의 예로, 가상 터미널(예를 들어, Telnet), JTM(Job transfer and Manipulation protocol, 표준 ISO/IEC 8832) 등이 있습니다.

- 사용자를 위한 인터페이스(UI, User Interface)를 제공하며, **사용자에게 보이는 유일한 계층**이라고 할 수 있습니다.
    - 컴퓨터나 스마트폰으로 인터넷을 사용할 때 직접 마주하는 계층입니다.

- 웹 브라우저, 이메일 클라이언트, 파일 전송 프로그램 등이 모두 응용 계층에서 동작하며, 사용자가 원하는 작업을 수행할 수 있도록 다양한 기능을 제공합니다. 
    - 웹 브라우저에서 웹 페이지를 열 때 **HTTP 프로토콜**을 사용합니다.
        - WWW(World Wide Web)를 위한 HTTP는 L7에서 동작하는 프로토콜입니다.
    - 이메일을 보낼 때 **SMTP 프로토콜**을, 이메일을 받을 때 **POP3 또는 IMAP 프로토콜**을 사용합니다.
    - 파일을 전송할 때 **FTP 프로토콜**을 사용합니다. 

- 응용 계층은 **포트 번호를 통해 전송 계층과 통신**합니다.
    - 포트 번호는 응용 프로그램들이 서로를 구별하고 통신하기 위해 사용하는 것으로, 주로 전송 계층에서 사용됩니다.
    - 응용 계층은 이 포트 번호를 직접 다루지는 않지만, **응용 계층 프로토콜**이 **전송 계층 프로토콜(TCP 또는 UDP)**을 통해 데이터를 주고받으며, 이때 **전송 계층 프로토콜이 포트 번호를 사용하여 통신**을 합니다.
    - 예를 들어, 웹 브라우저가 웹 서버에 접속할 때 응용 계층은 통신을 위해 간접적으로 포트 번호를 사용합니다.
        - 웹 브라우저(응용 계층)는 웹 서버에 접속할 때 HTTP 프로토콜(응용 계층)을 사용합니다.
        - HTTP 프로토콜은 TCP 프로토콜(전송 계층)을 통해 데이터를 주고받습니다.
        - TCP 프로토콜은 웹 서버의 80번 포트(well-known port)로 연결하여 통신을 합니다.
