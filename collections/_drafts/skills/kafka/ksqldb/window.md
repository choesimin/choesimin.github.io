...
Window는 시간을 기준으로 데이터를 그룹화하는 방법입니다. 실시간으로 흘러들어오는 데이터 스트림을 시간 단위로 잘라서 분석할 수 있게 해줍니다.

## 왜 Window가 필요할까요?

스트리밍 데이터는 계속해서 들어오기 때문에, 특정 시점의 집계나 분석이 어렵습니다. 예를 들어:

"최근 1시간 동안의 주문량은 얼마인가?"
"지난 30분간 가장 많이 팔린 상품은?"

이런 질문에 답하기 위해서는 시간을 기준으로 데이터를 나누어 처리해야 합니다. Window는 이런 시간 기반 처리를 가능하게 해주는 개념입니다.
...
이러한 공통 옵션들을 적절히 조합하여 사용하면 더욱 효과적인 윈도우 처리가 가능합니다.
