...
예를 들어, 온라인 쇼핑몰에서 두 명의 고객이 동시에 같은 상품을 구매하려고 할 때 갱신 분실이 발생할 수 있습니다. 재고가 1개인 상품을 두 명이 동시에 구매하려고 하는 상황을 가정해 보겠습니다.
...
데이터베이스->>고객A: 구매 완료
데이터베이스->>고객B: 구매 완료
Note over 데이터베이스: 최종 재고: 0개<br>(실제로는 -1개여야 함)
```

이 시나리오에서 발생하는 문제점은 다음과 같습니다:
