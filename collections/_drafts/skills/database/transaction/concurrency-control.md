...
sequenceDiagram
    ...
    고객A->>데이터베이스: 재고 확인 (1개)
    고객B->>데이터베이스: 재고 확인 (1개)
    고객A->>데이터베이스: 구매 요청 (재고 -1)
    고객B->>데이터베이스: 구매 요청 (재고 -1)
    데이터베이스->>고객A: 구매 완료
    데이터베이스->>고객B: 구매 완료
    Note over 데이터베이스: 최종 재고: 0개 (실제로는 -1개가 되어야 함)
```

**예시**: 온라인 쇼핑몰에서 두 명의 고객이 동시에 재고가 1개 남은 상품을 구매하려고 할 때 발생할 수 있는 시나리오를 살펴봅시다.

이 시나리오에서는 고객 A와 고객 B가 동시에 상품의 재고를 확인하고, 각각 재고 1개를 기반으로 구매 요청을 보냅니다. 데이터베이스는 두 요청을 처리하지만, 실제로는 한 요청만 처리되었어야 합니다. 이로 인해 두 번째 트랜잭션의 갱신이 첫 번째 트랜잭션의 갱신을 덮어쓰며, 최종 재고가 실제보다 잘못 저장됩니다.

1. 고객 A와 B가 동시에 재고를 확인하고 각각 구매를 시도합니다.
2. 두 고객 모두 재고가 1개인 것을 확인합니다.
3. 고객 A가 구매 요청을 보내고 재고를 1 감소시킵니다.
4. 고객 B도 구매 요청을 보내고 재고를 1 감소시킵니다.
5. 데이터베이스는 두 요청을 모두 처리하여 재고를 0으로 만듭니다.
6. 실제로는 재고가 -1이 되어야 하지만, 고객 B의 갱신이 고객 A의 갱신을 덮어쓰게 되어 최종 재고는 0이 됩니다.

이 경우, 고객 B의 구매로 인한 재고 감소가 '갱신 분실'되었습니다. 이로 인해 데이터베이스의 일관성이 깨지고, 실제 재고보다 더 많은 상품이 판매되는 문제가 발생합니다.

이러한 갱신 분실 문제를 해결하기 위해서는 트랜잭션 격리 수준을 적절히 설정하거나, 락(lock)을 사용하여 동시 접근을 제어하는 등의 동시성 제어 기법을 적용해야 합니다.
...
sequenceDiagram
    ...
    트랜잭션1->>데이터베이스: y에 500 더함
    트랜잭션2->>데이터베이스: y에 4를 곱함
    Note over 데이터베이스: 트랜잭션 2가 y에 4를 곱하기 전에 트랜잭션 1이 완료되어야 함
```

**예시**: 두 개의 트랜잭션이 데이터 `y`를 각각 갱신하는 상황을 살펴봅시다.

이 경우, 트랜잭션 1이 `y`에 500을 더하고, 그 이후에 트랜잭션 2가 `y`에 4를 곱해야 하지만 트랜잭션 2가 먼저 실행되어 `y`에 4를 곱한 후에 트랜잭션 1이 500을 더하는 상황이 발생할 수 있습니다. 이는 데이터의 일관성을 해치며, 트랜잭션의 결과가 서로 모순되는 문제를 야기합니다.
...
- 연쇄 복귀는 한 트랜잭션의 실패로 인해 다른 트랜잭션들이 함께 롤백되는 현상입니다.
    - 병행 수행 중인 트랜잭션들이 서로 참조하고 있거나 상호 연관된 경우, 하나의 트랜잭션이 실패하면 다른 트랜잭션들도 영향을 받아 롤백될 수 있습니다.



**예시**: 트랜잭션 1이 데이터를 갱신하고, 그 데이터를 기반으로 트랜잭션 2가 수행되는 상황을 가정해 봅시다.
...
sequenceDiagram
    ...
    트랜잭션1->>데이터베이스: 데이터 갱신
    트랜잭션2->>데이터베이스: 갱신된 데이터 참조
    트랜잭션1->>데이터베이스: 트랜잭션1 실패로 인해 롤백
    트랜잭션2->>데이터베이스: 트랜잭션2도 롤백
```

이 상황에서 트랜잭션 1이 실패하여 롤백되면, 트랜잭션 2는 이미 트랜잭션 1이 갱신한 데이터를 참조하고 있기 때문에 트랜잭션 2도 롤백되어야 합니다. 이러한 연쇄적인 롤백은 데이터 일관성 문제로 이어집니다.


### 4. 현황 파악 오류 (Uncommitted Dependency)

- 현황 파악 오류는 한 트랜잭션이 아직 커밋되지 않은 데이터를 다른 트랜잭션이 참조하면서 발생하는 문제를 의미합니다.
    - 중간 상태의 데이터에 의존하여 잘못된 결과를 도출할 수 있는 위험을 동반합니다.

**예시**: 트랜잭션 1이 데이터를 갱신하고 있지만 아직 커밋되지 않은 상태에서 트랜잭션 2가 그 데이터를 참조하는 상황을 가정해 봅시다.
...
sequenceDiagram
    ...
    트랜잭션1->>데이터베이스: 데이터 갱신 (미완료)
    트랜잭션2->>데이터베이스: 미완료 데이터를 참조
    트랜잭션1->>데이터베이스: 트랜잭션 1 실패
    Note over 트랜잭션2: 트랜잭션2는 잘못된 데이터를 참조하여 잘못된 결과 도출
```

트랜잭션 1이 실패한 후에도 트랜잭션 2는 이미 실패한 데이터를 기반으로 연산을 수행했기 때문에, 트랜잭션 2 역시 오류를 내포할 수밖에 없습니다. 이러한 문제는 트랜잭션의 무결성과 신뢰성을 저하시킵니다.
