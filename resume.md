...
결제선생 서비스와 관리자 페이지의 **웹 보안 취약점을 점검하고 조치**하였습니다. 비밀번호, 민감 정보, 셰션 등에 대한 관리 방법을 개선하여, **서비스 전반의 보안 수준을 강화**하였습니다.
...
- 세션에 저장해두고 사용하는 **고정된 식별값이 쉽게 유추가 가능한 취약점**을 개선하였습니다. 동일한 문자열 내의 일부만 다르게 관리되어 적은 시도만으로 알아낼 수 있었기 때문에, 식별값이 더 복잡한 값을 가질 수 있도록 **문자열 생성 규칙을 변경**하였습니다. 이를 위해 랜덤 문자열 생성 모듈을 새로운 Java UUID 생성 라이브러리로 교체하였습니다.
